...
def load_prior_revenue():
    """
    从上期 extract 文件的「表14」sheet 读取上月收益。
    返回 {分公司: 上月收益} 的字典。
    """
    ...
    try:
        df = pd.read_excel(PRIOR_EXTRACT, sheet_name='表14')
        result = {}
        for _, row in df.iterrows():
            branch = str(row.get('分公司', '')).strip()
            revenue = parse_num(row.get('本月收益（元）', float('nan')))
            if branch and not pd.isna(revenue):
...
def process():
    ...
    # 5. 加载上月收益
    prior_revenue = load_prior_revenue()
    ...
    for branch, bp_total in bp_data:
        ...
        # BP完成率
        if pd.isna(bp_total) or bp_total == 0 or pd.isna(this_revenue):
            bp_rate = '/'
        else:
            bp_rate = f"{this_revenue / bp_total:.2%}"
